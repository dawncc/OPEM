...
除召回排序反馈外，系统现在会从每个用户轮次派生 `TaskRun` 和 `ExecutionNode`，以强/中/弱三级 `OutcomeEvidence` 汇总任务质量，并分别记录路径相关性、冗余、验证价值和成本。没有真实消融时，必要性保持未知；配对回放和随机消融通过独立接口提交。策略版本、Shadow、默认关闭的低风险 Canary、历史回填、离线报告和 `/evolution` 管理页已接入。完整协议与运行方式见 [零样本执行路径自演进](execution-path-evolution.md)。
